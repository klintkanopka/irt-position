...
resp$pi<-invlogit(((n/2)-resp$sequence_number)/3)
...
write.csv(x, file='data3.csv', row.names=FALSE)




############################################
#two categories of person-level change
##uniform change
set.seed(1234120101)
N<-1000
n<-50
...
resp$k<-ifelse(resp$id %% 2==0,40,10)
resp$pi<-invlogit((resp$k-resp$sequence_number)/3)
resp$diff.early<-resp$diff-.2
resp$diff.late<-resp$diff+.2
...
## resp - a dichotomously coded response indicator, coded 0 if incorrect and 1 if correct
x<-resp[,c("id","itemkey","sequence_number","resp")]
write.csv(x, file='data4.csv', row.names=FALSE)
